...
export class AIHandler {
    ...
    public synthIdChecker: SynthIdChecker;
    ...
    constructor(app: App) {
        ...
        this.synthIdChecker = new SynthIdChecker(this.auth, config);
    ...
    public async processAIQuestion(question: string | Part[], history: Content[], channelId: string, threadTs?: string, userId?: string): Promise<AIResponse> {
        ...
        while (retries > 0) {
            try {
                ...
                const synthIdTool: LLMTool = {
                    name: "check_synthid_watermark",
                    description: "Checks an image for digital Google SynthID watermarks to determine if it was generated by Google AI models. Call this tool whenever a user asks if an image is AI-generated, alongside qualitative image analysis.",
                    parameters: {
                        type: "object",
                        properties: {
                            image_url: {
                                type: "string",
                                description: "The URL of the image to check. If omitted, the tool automatically checks the latest attached image in the conversation thread.",
                            },
                        },
                        required: [],
                    },
                ...
                tools.push(slackTool, webTool, imageTool, searchMemesTool, generateMemeTool, reminderTool, synthIdTool);
...
`;
                ...
                while (currentResponse.toolCalls && currentResponse.toolCalls.length > 0 && iteration < maxIterations) {
                    ...
                    for (const tc of toolCalls) {
                        ...
                        try {
                            const sideEffectTools = ['generate_image', 'update_rpg_context', 'set_reminder', 'check_synthid_watermark'];
    ...
    public async executeTool(name: string, args: any, channelId: string, threadTs?: string, userId?: string): Promise<Part> {
        if (name.includes('__')) {
            ...
            if (name.startsWith('youtube__')) {
                ...
                return executeTool(this.app, this.imageGenerator, nativeName, args, channelId, threadTs, userId, this.synthIdChecker);
            }
        ...
        return executeTool(this.app, this.imageGenerator, name, args, channelId, threadTs, userId, this.synthIdChecker);
    }
